const imagesloaded = require('./imagesloaded.pkgd.min.js');
// const charming = require('./charming.min.js');
const charming = require('./charming.js');
...
const {gsap} = require("gsap")
	...
	class GridItem {
		constructor(el) {
			...
			this.DOM.tilt.title = this.DOM.el.querySelector('.grid__item-role');
			...
			this.tiltconfig = {   
                title: {translation : {x: [-12,8], y: [8,-8]}},
                number: {translation : {x: [-9,9], y: [-14,14]}},
                img: {translation : {x: [-19,19], y: [-14,14]}}
			};
					...
					gsap.to(letter, {
						...
						onComplete: () => {
							gsap.to(letter, {
								...
								y: '0%',
								opacity: 1,
								duration: type === 'mouseenter' ? 0.3 : .6
							});
						}
					});
				});
		...
		toggle(withAnimation, show = true) {
			gsap.to(this.DOM.tilt.img, {
				ease: "Expo.easeInOut",
				delay: !withAnimation ? 0 : show ? 0.15 : 0,
				scale: show ? 1 : 0,
				opacity: show ? 1 : 0,
				duration: withAnimation ? 0.8 : 0,
			});
			gsap.to(this.DOM.bg, {
				ease: "Expo.easeInOut",
		...
		toggleTexts(delay, withAnimation, show = true) {
			gsap.to([this.DOM.tilt.title, this.DOM.tilt.number], {
				ease: show ? "Expo.easeOut" : "Quart.easeIn",
				delay: !withAnimation ? 0 : delay,
				y: show ? 0 : 20,
				opacity: show ? 1 : 0,
				duration: !withAnimation ? 0 : show ? 1 : 0.5
			});
		}
	}
    ...
    class Grid {
		...
		openItem(item) {
			...
			const bg = document.querySelectorAll(".grid__item-bg-g");
			const itemDim = this.getSizePosition(bg);
		...
		toggleContentElems(contentEl, delay, withAnimation, show = true) {
			...
			gsap.to([this.DOM.closeCtrl, this.DOM.scrollIndicator], {
